...
class CodeBuddyAPIClient:
    ...
    def convert_openai_to_codebuddy_messages(
        ...
        for msg in openai_messages:
            ...
            if (
                ...
                and ("Error: API error" in content or "API error:" in content)
            ):
                continue
        ...
        for msg in filtered_messages:
            ...
            logger.debug(
                f"[DEBUG] Processing message - role: {role}, content type: {type(content)}"
            )
            ...
            if has_tool_content:
                # 包含工具调用内容，保持结构化格式
                logger.info(
                    f"[TOOL_CONTENT] Preserving structured content for role: {role}"
                )
                    ...
                    if isinstance(item, dict):
                        if item.get("type") == "tool_result":
                            ...
                            if not tool_use_id:
                                ...
                                logger.warning(
                                    f"[TOOL_RESULT] Missing toolUseId, generated: {tool_use_id}"
                                )

                            # 确保toolUseId符合正则表达式要求 [a-zA-Z0-9_-]+
                            if not tool_use_id or not all(
                                c.isalnum() or c in "_-" for c in tool_use_id
                            ):
                                tool_use_id = f"tool_{uuid.uuid4().hex[:8]}"
                                logger.warning(
                                    f"[TOOL_RESULT] Invalid toolUseId format, regenerated: {tool_use_id}"
                                )
                            ...
                            logger.info(
                                f"[TOOL_RESULT] Processed tool result with toolUseId: {tool_use_id}"
                            )
                            ...
                            logger.info(
                                f"[TOOL_USE] Processed tool use with id: {tool_id}"
                            )
                            ...
                            if "text" in item and not item.get("type"):
                                ...
                                processed_content.append(tool_result)
                                logger.info(
                                    f"[TOOL_RESULT] Converted text item to tool result with toolUseId: {tool_use_id}"
                                )
                            else:
                                processed_content.append(item)
                    else:
                ...
                codebuddy_msg = {"role": role, "content": processed_content}
            else:
                # 普通文本内容，转换为字符串
                ...
                codebuddy_msg = {"role": role, "content": text_content}
    ...
    def generate_codebuddy_headers(
        ...
        headers = {
            ...
            "Content-Type": "application/json",
            "X-Requested-With": "XMLHttpRequest",
            # "X-Trace-ID": "bf0f9c95d9160a034187724fd598aa9a",
            # "X-Request-ID": "db96f0077ff94b1ca6c166841bdaeb42",
            # "b3": "bf0f9c95d9160a034187724fd598aa9a-921aa13f67aad8e7-1-",
            # "X-B3-TraceId": "bf0f9c95d9160a034187724fd598aa9a",
            "X-B3-ParentSpanId": "",
            # "X-B3-SpanId": "921aa13f67aad8e7",
            "X-B3-Sampled": "1",
            # "X-Conversation-ID": "db96f0077ff94b1ca6c166841bdaeb42",
            # "X-Conversation-Request-ID": "db96f0077ff94b1ca6c166841bdaeb42",
            # "X-Conversation-Message-ID": "db96f0077ff94b1ca6c166841bdaeb42",
            # "X-Session-ID": "5909854651654b1a9611f99590f479f51756267819935",
            "X-Agent-Intent": "CodeCompletion",
            "X-Env-ID": "production",
            "Authorization": f"Bearer {bearer_token}",
            "X-User-Id": "56277846-534b-4f8d-b8b4-9793936bc07c",
            "X-Domain": "copilot.tencent.com",
            # "User-Agent": "CodeBuddyIDE/0.1.27 CodeBuddy/3.2.4441357 ",
            "User-Agent": "",
            "X-Product": "SaaS",
